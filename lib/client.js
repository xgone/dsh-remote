	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			if (connection !== void 0 && connection.isLoopback === false) {
				...
				// Welcome notice (内测声明 / Internal Testing Notice): the same
				// loopback gate — WelcomeNoticeStore runs in memory mode for remote
				// browsers, so load() never reads the ack and acknowledge() never
				// writes it; the notice re-pops on every refresh. Reach the live
				// instance through the settings.onboarding slot's inject (which
				// returns { controller }) and flip its persistence to "host": the
				// store's own load()/acknowledge() then run the real Host path (real
				// settings.describe/mutate, real version constant), so a remote user
				// acknowledges ONCE and the notice stays dismissed — including when
				// DSH bumps the welcome version. Re-load so the just-flipped store
				// re-reads the persisted ack immediately.
				try {
					const welcomeEntry = ctx.slots.entries("settings.onboarding")
						.find((entry) => entry.options?.id === "welcome-notice");
					const wc = welcomeEntry?.options?.inject?.()?.controller;
					if (wc && wc.persistence === "memory") {
						wc.persistence = "host";
						wc.load?.();
					}
				} catch (error) {
					// slot or instance shape drifted — leave the notice as DSH ships it
				}
			}
