	...
	factory: (require) => {
		...
		const copy = {
			...
			en: {
				...
				"viewer.renderError": "Render failed:",
		...
		function FileBody({ path, view }) {
			...
			if (view.status === "text" && view.markdown && MarkdownText) {
				return h("div", null, [
					...
					// `labels` is REQUIRED by MarkdownText: the fence renderer
					// reads `labels.code.copyLabel` (footnotes read
					// `labels.footnotes`). Omitting it made every markdown file
					// containing a code fence throw mid-render and unmount the
					// whole overlay root — the panel "flashed and vanished".
					jsx(MarkdownText, { text: view.text, labels: markdownLabels() })
				]);
			}
			if (view.status === "text") {
				return h("div", null, [
		...
		/** Localized chrome MarkdownText requires for fence copy buttons and
		 *  the footnotes section. Exported for unit tests. */
		...
		/** Keeps one pane's renderer crash from unmounting the WHOLE overlay
		 *  root (which also takes the auth gate down with it). */
		class PaneErrorBoundary extends react.Component {
			...
			render() {
				if (this.state.error !== null) {
					const detail = this.state.error && this.state.error.message ? this.state.error.message : String(this.state.error);
					return h("div", { style: mutedStyle }, t("viewer.renderError") + " " + detail);
				}
		...
		function FilePane({ pane, maximized, collapsed, onMaximize, onClose }) {
			...
			return h("div", {
				...
				}, h(PaneErrorBoundary, null, h(FileBody, { path: pane.path, view })))
			]);
		}
		...
		// "settingsScope" is intentionally NOT declared: dsh 0.1.7's settings migration
		// removed that client service (split into "settingsSchema" + "configForms" in
		// @deepseek-ai/dsh-client-ui-settings), so declaring it kept this entry pending
		// forever on dsh >= 0.1.7 (upstream issue #20). The only consumer below reaches
		// the describe mirror through the optional-chained ctx.configForms?.describe?.()
		// face instead -- a missing service simply no-ops, exactly like a drifted mirror
		// shape always has.
		...
		function apply(ctx) {
			...
			// Snapshot the deployment's real topology BEFORE the alpha flip below
			// mutates the shared connection facts: the remote-browser corrections
			// further down must key off the un-flipped value. Reading it back
			// after the flip made this gate see `isLoopback === true` on every
			// non-loopback page, silently skipping locale persistence and
			// re-adoption, the settings-scope unpin + refresh, the welcome-notice
			// ack backfill and the models describe mirror (issue #16).
			let connection = void 0;
			try {
				connection = ctx.get("connection");
			} catch (error) {
				// connection service unavailable — corrections keyed off it are
				// skipped and the page stays on DSH's native remote behavior
			}
			const remoteConnection = connection !== void 0 && connection.isLoopback === false;
			if (!isLoopbackPage()) {
				...
				if (remoteConnection) connection.isLoopback = true;
			...
			if (remoteConnection) {
				...
				// is reached through the official describe() face: ctx.configForms on
				// dsh >= 0.1.7 (ctx.settingsScope before the settings migration)).
		...
		exports.name = name;
		/** Test-only surfaces: the wire-shape matcher, the fetch patch, the
		 *  file-kind classifier and the MarkdownText labels builder. */
