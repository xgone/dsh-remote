...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** User icon from the shell's primitives set (cosmetic; optional). */
		let IconUserOutline16 = null;
		try {
			const primitives = require("@deepseek-ai/dsh-client-ui-primitives");
			IconUserOutline16 = primitives.IconUserOutline16 ?? null;
		} catch (error) {
			// icons are cosmetic — fall back to text-only rendering
		}
		...
		function apply(ctx) {
			...
			if (connection !== void 0 && connection.isLoopback === false) {
				...
				settingsRpc("settings.describe", {}).then((result) => {
					...
					console.warn("[dsh-remote] welcome ack =", JSON.stringify(ack), "| describe result ok =", !!result?.ok);
					if (typeof ack !== "string" || ack.length === 0) {
						console.warn("[dsh-remote] welcome: no ack, skipping (first-time remote user)");
						return;
					}
					try {
						const entries = ctx.slots.entries("settings.onboarding");
						const onboardingEntry = entries?.find((entry) => entry.options?.id === "welcome-notice");
						console.warn("[dsh-remote] onboarding entries =", entries?.length, "| welcome entry found =", !!onboardingEntry, "| ids =", entries?.map((e) => e.options?.id));
						let injected;
						// ui-slots stores inject as a TOP-LEVEL entry property (not under .options)
						try { injected = onboardingEntry?.inject?.() ?? onboardingEntry?.options?.inject?.(); } catch (e) { console.warn("[dsh-remote] inject() threw:", e.message); }
						const store = injected?.hooks?.welcome;
						console.warn("[dsh-remote] store found =", !!store, "| has update =", typeof store?.update, "| has getSnapshot =", typeof store?.getSnapshot, "| acknowledged =", store?.getSnapshot?.()?.acknowledged);
						if (store && typeof store.update === "function" && typeof store.getSnapshot === "function") {
							if (!store.getSnapshot().acknowledged) {
								store.update((snapshot) => { snapshot.acknowledged = true; });
								console.warn("[dsh-remote] welcome: set acknowledged=true ✓");
							} else {
								console.warn("[dsh-remote] welcome: already acknowledged, no-op");
							}
						}
					} catch (error) {
						console.warn("[dsh-remote] welcome block threw:", error?.message, error?.stack?.slice(0, 200));
					}
				});
			}
			...
			const renderGate = () => root.render(h(AuthGate, {}));
			renderGate();
