	...
	factory: (require) => {
		...
		const copy = {
			...
			en: {
				...
				"viewer.binary": "This file type cannot be previewed in the viewer",
		...
		// (FileViewerModal below) backed by /auth/file on the host half: the
		...
		/** Tiny pub/sub the fetch patch uses to open the viewer without React. */
		const fileViewerStore = {
			state: { path: null },
			listeners: new Set(),
			open(path) {
				fileViewerStore.state = { path };
				for (const listener of fileViewerStore.listeners) listener();
			},
			close() {
				fileViewerStore.state = { path: null };
				for (const listener of fileViewerStore.listeners) listener();
			},
			subscribe(listener) {
		...
		function FileViewerModal() {
			...
			const close = react.useCallback(() => fileViewerStore.close(), []);
			react.useEffect(() => {
			...
			react.useEffect(() => {
				if (path === null) { setView(null); return undefined; }
				let cancelled = false;
				let objectUrl = null;
				...
				(async () => {
					try {
						...
						if (!response.ok) {
							...
							if (!cancelled) setView({ status: "error", error: (message && message.error) || `HTTP ${response.status}` });
							return;
						}
			...
			return h("div", {
				style: {
					...
					background: "var(--dsw-overlay, rgba(8,10,14,.72))", backdropFilter: "blur(4px)",
				...
				h("div", {
					style: {
						width: "100%", maxWidth: "860px", maxHeight: "86vh", boxSizing: "border-box",
						display: "flex", flexDirection: "column",
						background: "var(--dsw-specific-menu, #161b22)",
						border: "1px solid var(--dsw-alias-border-l2, #30363d)",
						borderRadius: "12px", boxShadow: "0 12px 32px rgba(0,0,0,.35)",
						overflow: "hidden"
					},
					...
					h("div", {
						style: {
							...
							padding: "12px 16px", borderBottom: "1px solid var(--dsw-alias-border-l2, #30363d)",
							flex: "none"
						}
					}, [
						h("span", {
							style: { flex: 1, minWidth: 0, fontSize: "14px", fontWeight: 600,
					...
					h("div", {
						style: {
							flex: 1, minHeight: 0, overflow: "auto", padding: "16px",
							color: "var(--dsw-alias-label-primary, #e6edf3)", fontSize: "13px", lineHeight: "1.7"
						}
					}, [
						view === null || view.status === "loading"
							? h("div", { style: mutedStyle }, "…")
							: view.status === "error"
								? h("div", { style: { color: "var(--dsw-danger, #f85149)" } }, view.error)
								: view.status === "image"
									? h("img", { src: view.url, alt: fileName, style: { maxWidth: "100%", maxHeight: "62vh", display: "block", margin: "0 auto", borderRadius: "8px" } })
									: view.status === "embed"
										? (view.kind === "video"
											? h("video", { src: view.url, controls: true, style: { maxWidth: "100%", maxHeight: "62vh", display: "block", margin: "0 auto" } })
											: h("iframe", { src: view.url, title: fileName, style: { width: "100%", height: "62vh", border: "0", borderRadius: "8px", background: "#fff" } }))
														...
														: view.data.entries.map((entry) =>
															h("button", {
																key: entry.path, style: {
																	...
																	border: "0", color: "var(--dsw-accent, #4c8bf5)",
																	font: "inherit", padding: "4px 6px", borderRadius: "6px"
																},
																onClick: () => fileViewerStore.open(entry.path),
																onMouseEnter: (event) => { event.currentTarget.style.background = "var(--dsw-interactive-bg-hover, rgba(255,255,255,.06))"; },
																onMouseLeave: (event) => { event.currentTarget.style.background = "transparent"; }
															}, (entry.isDirectory ? "📁 " : "📄 ") + entry.name + (entry.isDirectory ? "/" : "")))
												])
											: view.status === "text" && view.markdown && MarkdownText
												? jsx(MarkdownText, { text: view.text })
												: view.status === "text"
													? h("pre", { style: { margin: "0", whiteSpace: "pre-wrap", wordBreak: "break-word", fontFamily: "var(--dsw-font-mono, monospace)", fontSize: "12.5px" } }, view.text)
		...
		function TotpTtl({ remaining }) {
			...
			return h("div", {
				style: {
					marginTop: "6px",
					fontSize: "12px",
					color: remaining <= 5 ? "var(--dsw-danger, #f85149)" : "var(--dsw-alias-label-tertiary, #8b949e)"
				}
		...
		const fieldStyle = {
			...
			borderRadius: "8px",
			border: "1px solid var(--dsw-alias-border-l2, #30363d)",
			background: "var(--dsw-alias-fill-l2, #0d1117)",
			color: "var(--dsw-alias-label-primary, #e6edf3)",
			fontSize: "13px",
			outline: "none"
		};
		const buttonStyle = {
			...
			borderRadius: "8px",
			border: "0",
			background: "var(--dsw-accent, #4c8bf5)",
			color: "#fff",
			fontSize: "13px",
			cursor: "pointer"
		};
		...
		const chipStyle = {
			...
			borderRadius: "5px",
			fontSize: "11px",
			background: "var(--dsw-alias-fill-l2, #21262d)",
			color: "var(--dsw-alias-label-secondary, #8b949e)"
		};
		const labelStyle = {
			display: "block",
		...
		function LoginOverlay({ me, onDone }) {
			...
			const overlayStyle = {
				...
				background: "var(--dsw-overlay, rgba(8,10,14,.82))",
			...
			return h("div", { style: overlayStyle },
				h("form", { onSubmit: submit, style: cardStyle },
					...
					error
						? h("div", { style: { marginTop: "12px", padding: "8px 10px", borderRadius: "8px", fontSize: "13px", color: "var(--dsw-danger, #f85149)", background: "rgba(248,81,73,.12)", border: "1px solid rgba(248,81,73,.35)" } }, error)
						: null,
					h("button", {
						type: "submit",
		...
		function MfaSelfService({ me, onChanged }) {
			...
			if (view.phase === "setup" && view.setup) {
				...
				return h("div", null, [
					...
					h("div", { style: { display: "flex", gap: "6px", alignItems: "center" } }, [
						h("code", { style: { flex: 1, padding: "8px 10px", borderRadius: "8px", background: "var(--dsw-alias-fill-l2, #0d1117)", border: "1px solid var(--dsw-alias-border-l2, #30363d)", fontFamily: "var(--dsw-font-mono, monospace)", fontSize: "12px", userSelect: "all" } }, setup.secret),
					...
					h("div", { style: { display: "flex", gap: "6px", alignItems: "center" } }, [
						h("code", { style: { flex: 1, padding: "8px 10px", borderRadius: "8px", background: "var(--dsw-alias-fill-l2, #0d1117)", border: "1px solid var(--dsw-alias-border-l2, #30363d)", fontFamily: "var(--dsw-font-mono, monospace)", fontSize: "11px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, setup.otpauth),
					...
					h("div", { style: { display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "6px" } },
						(setup.backupCodes || []).map((code) =>
							h("code", { key: code, style: { padding: "3px 8px", borderRadius: "6px", background: "var(--dsw-alias-fill-l2, #21262d)", fontFamily: "var(--dsw-font-mono, monospace)", fontSize: "12px" } }, code)
						)),
			...
			return h("div", null, [
				...
				h("div", { style: { display: "flex", alignItems: "center", gap: "10px", fontSize: "13px", color: "var(--dsw-alias-label-secondary, #c9d1d9)" } }, [
					t("mfa.status"),
					h("span", { style: { ...chipStyle, background: me.mfa ? "rgba(63,185,80,.15)" : "var(--dsw-alias-fill-l2, #21262d)", color: me.mfa ? "#3fb950" : "var(--dsw-alias-label-secondary, #8b949e)" } },
				...
				me.mfa
					? h("form", { onSubmit: disable, style: { marginTop: "10px" } }, [
						...
						h("div", { style: { display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "8px" } }, [
							...
							h("button", { type: "submit", disabled: !view.password || view.code.trim().length < 6, style: { ...ghostButtonStyle, color: "var(--dsw-danger, #f85149)" } }, t("mfa.disable"))
						]),
						view.message ? h("div", { style: { ...mutedStyle, marginTop: "8px" } }, view.message) : null
					])
							...
							: h("div", { style: { display: "flex", flexDirection: "column", gap: "10px", marginTop: "10px" } },
								view.accounts.map((account) =>
									h("div", { key: account.username, style: {
										background: "var(--dsw-alias-fill-l2, #161b22)",
										border: "1px solid var(--dsw-alias-border-l2, #30363d)",
										borderRadius: "12px",
										padding: "14px 16px",
										display: "flex",
										...
										h("div", { style: { display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" } }, [
											...
											h("span", { style: { ...chipStyle, background: "rgba(76,139,245,.14)", color: "var(--dsw-accent, #4c8bf5)" } },
										...
										h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" } }, [
											...
											account.protected
												...
												: h("button", {
													style: { ...ghostButtonStyle, color: "var(--dsw-danger, #f85149)" },
				...
				me.authenticated
					? h("div", { style: { borderTop: "1px solid var(--dsw-alias-border-l2, #30363d)", paddingTop: "16px" } },
						h("button", {
							...
							style: { ...ghostButtonStyle, color: "var(--dsw-danger, #f85149)", padding: "7px 14px", display: "inline-flex", alignItems: "center", gap: "6px" },
		...
		function apply(ctx) {
			...
			if (connection !== void 0 && connection.isLoopback === false) {
				// Redirect file clicks to /auth/file so they display in THIS
				// browser instead of opening on the host desktop (see
			...
			const renderGate = () => root.render(h("div", null, [h(AuthGate, { key: "gate" }), h(FileViewerModal, { key: "viewer" })]));
			renderGate();
