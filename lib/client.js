...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function FileBody({ path, view }) {
			...
			if (view.status === "error") return h("div", { style: { color: "var(--dsw-danger, #f85149)" } }, view.error);
		...
		function FilePane({ pane, maximized, collapsed, onMaximize, onClose }) {
			...
			return h("div", {
				...
				h("div", {
					...
					h("button", {
						style: { ...ghostButtonStyle, color: maximized ? "var(--dsw-accent, #4c8bf5)" : "var(--dsw-alias-label-secondary, #8b949e)" },
		...
		function SidePanelHost() {
			...
			return h("div", {
				...
				h("div", {
					...
					onMouseEnter: (event) => { event.currentTarget.style.background = "var(--dsw-accent, #4c8bf5)"; },
					onMouseLeave: (event) => { event.currentTarget.style.background = "transparent"; },
