...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── AbortSignal polyfill (mobile/iOS WebKit) ─────────────────────────────
		// DSH's client connection layer merges its RPC timeout with the caller's
		// abort signal via `AbortSignal.any([AbortSignal.timeout(ms), signal])`
		// (client/connection postJson). On Apple-device Chrome — which is really
		// WKWebView — and on Safari before 17.4, `AbortSignal.timeout` exists but
		// `AbortSignal.any` does not, so EVERY message RPC throws
		// "AbortSignal.any is not a function" and the send dies before the
		// request leaves the tab. DSH source cannot be touched here, so install a
		// spec-compatible `AbortSignal.any` (and a safety-net `.timeout`) at
		// module load — before any request path can run. Empty array → a signal
		// that never aborts; already-aborted inputs short-circuit with their
		// reason, mirroring the composed-any contract.
		(function installAbortSignalPolyfill() {
			try {
				if (typeof AbortSignal === "undefined") return;
				if (typeof AbortSignal.timeout !== "function") {
					AbortSignal.timeout = function (ms) {
						var controller = new AbortController();
						var reason = new DOMException("The operation timed out.", "TimeoutError");
						setTimeout(function () {
				...
				if (typeof AbortSignal.any !== "function") {
					AbortSignal.any = function (signals) {
						var controller = new AbortController();
						var settled = false;
						var onAbort = function (signal) {
							if (settled) return;
							settled = true;
							try { controller.abort(signal && signal.reason); } catch (e) { controller.abort(); }
						};
						try {
							const list = Array.from(signals || []);
							for (let i = 0; i < list.length; i += 1) {
								const signal = list[i];
								if (!signal || typeof signal.aborted !== "boolean") continue;
								if (signal.aborted) { onAbort(signal); return controller.signal; }
								signal.addEventListener("abort", function () { onAbort(signal); }, { once: true });
							}
						} catch (error) {
				...
				// polyfill failure must never break boot — DSH keeps its own path
			}
		})();
		...
		/** Largest the host may be on this viewport: on a narrow (phone) screen it
		 *  must not overflow past the visible edge, so the default 460px column
		 *  clamps to just inside the viewport and the drag range follows suit.
		 *  Desktop keeps the usual 300–720 band. */
		function panelWidthRange() {
			const viewport = typeof window !== "undefined" ? Math.max(300, window.innerWidth - 8) : 720;
			return { min: 300, max: Math.min(720, viewport) };
		}
		...
		function SidePanelHost() {
			...
			const [width, setWidth] = react.useState(() => Math.min(460, panelWidthRange().max));
			const startResize = react.useCallback((event) => {
				...
				const onMove = (e) => {
					const next = Math.min(max, Math.max(min, startWidth + (startX - e.clientX)));
					setWidth(next);
				};
				const onUp = () => {
			...
			return h("div", {
				style: {
					...
					width: width + "px", maxWidth: "100vw", zIndex: "2147482990", pointerEvents: "auto",
					display: "flex", flexDirection: "column",
		...
		function LoginOverlay({ me, onDone }) {
			...
			// On a phone the on-screen keyboard eats half the viewport; shrink the
			// inset and cap the card to the remaining height so the fields stay
			// reachable (the card scrolls internally instead of overflowing
			// behind the keyboard). Desktop keeps the roomier 24px inset. Use the
			// dynamic viewport unit when the browser offers it so the cap tracks
			// the keyboard's resized viewport, not the large (unkeyed) height.
			const viewportW = typeof window !== "undefined" ? window.innerWidth : 9999;
			const inset = viewportW <= 480 ? 12 : (viewportW <= 768 ? 16 : 24);
			const vhUnit = (typeof CSS !== "undefined" && typeof CSS.supports === "function" && CSS.supports("height", "100dvh"))
				? "100dvh" : "100vh";
			const overlayStyle = {
				position: "fixed",
				inset: "0",
			...
			const cardStyle = {
				...
				padding: "26px 24px",
				maxHeight: "calc(" + vhUnit + " - " + (inset * 2) + "px)",
				overflowY: "auto",
		...
		function apply(ctx) {
			...
			// ── mobile shell polish (DSH-native, injected; not DSH source) ────────
			// DSH is desktop-first, so on narrow (phone) viewports its own chrome
			// can collide: the session header crams the breadcrumb title, action
			// pills and the utility pill (mode / session log) into one row, and the
			// view tabs can be clipped. This plugin cannot edit DSH, so it injects a
			// narrow-viewport stylesheet that lets the title row wrap and the tabs
			// scroll instead of overlapping. Scoped to <=767px (phones + small
			// tablets in portrait); desktop and DSH's own >1024 breakpoint behaviour
			// are untouched. Selectors are structural ([data-phase] is the resident
			// conversation root, [role=tablist] the view tabs, [data-composer-card]
			// the composer card) so they survive DSH's hashed CSS-module class names.
			// Best-effort: a future DSH that renames these attributes simply stops
			// matching, and the status quo ante (whatever it renders) remains.
			const mobileShellFix = document.createElement("style");
			mobileShellFix.dataset.plugin = "dsh-remote";
			mobileShellFix.textContent = [
				"@media (max-width: 767px) {",
				"  /* session header: let the title row wrap instead of crushing the",
				"     breadcrumbs against the utility pill; the tabs scroll sideways */",
				"  [data-phase] > header { padding-left: 12px; padding-right: 12px; padding-top: 10px; }",
				"  [data-phase] > header > div:first-child { flex-wrap: wrap; min-height: auto; gap: 8px 12px; }",
				"  [data-phase] > header [role='tablist'] { gap: 24px; max-width: 100%; overflow-x: auto; }",
				"  /* composer: keep the tool row wrapping and let the model pill",
				"     truncate so the send button never gets pushed off-screen */",
				"  [data-composer-card] > div:last-child { flex-wrap: wrap; gap: 8px; }",
				"  [data-composer-card] > div:last-child > div:last-child { min-width: 0; }",
				"}"
			].join("\n");
			document.head.appendChild(mobileShellFix);
			...
			ctx.effect(() => () => {
				...
				if (mobileShellFix.parentNode) mobileShellFix.parentNode.removeChild(mobileShellFix);
