...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// /api fetch carrying that RPC and instead open /auth/file?path=… (the
		// host half of this plugin streams the file / renders a directory
		// listing) in a new tab. The RPC itself is answered locally with the
		// same ok envelope the host would have returned, so callers'
		// `.catch(() => {})` paths stay quiet. Loopback browsers keep DSH's
		// native behavior. Patched at the fetch layer (not the runtime client)
		// so every consumer (composer, artifacts, workspace tree) is covered
		...
		function installRemoteFileOpen() {
			...
			window.fetch = function (input, init) {
				try {
					...
					window.open("/auth/file?path=" + encodeURIComponent(path), "_blank", "noopener");
