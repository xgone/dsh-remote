	...
	*/
	...
	const handleFile = (req, res) => {
		...
		if (!verdict.ok) {
			...
			// immediately knows what to add to files.roots in the config.
			const extra = verdict.reason === "outside-roots"
				? { allowedRoots: fileRootsFor(cfg.files.roots), hint: "add the file's directory to the plugin's files.roots config" }
				: undefined;
