...
- **修复（issue #16）：经反向代理以非回环地址打开时，界面语言不再恢复记录，设置中「模型」「插件」等页面显示异常或为空**。0.3.2 为让 dsh ≥ 0.1.2-alpha 渲染产物文件「打开」入口（issue #11），在远程页面把 connection 事实源的 `isLoopback` 翻转为 `true`；但翻转发生在随后读取同一事实源、用于判定「远程浏览器修正块」的判断之前，导致该判断在一切非回环页面上恒为假——语言偏好的持久化与启动回读、远程设置作用域解锁 + `/auth/settings-refresh` 重播、欢迎声明已读回填、模型页 describe 镜像注水等整块修正从未运行。现改为在翻转前先快照连接事实的真实拓扑，修正块按快照判定；issue #11 的翻转行为保持不变。
