...
**移动端体验优化**（DSH 桌面优先，窄视口由本插件补齐）：

- **回车不再误发**：触屏（`pointer: coarse`）设备上，聊天输入框的**回车**改成换行而不是发送——手机上键盘的回车极易误触发送；发送走发送按钮（或物理键盘的修饰键 + 回车，硬件键盘仍可用）。
- **窄屏布局注入**（≤767px）：DSH 会话头部会把面包屑标题、动作按钮、模式/会话日志工具按钮挤在一行而互相重叠，视图标签也可能被裁切。插件注入窄屏样式让标题行**换行**、视图标签**横向滚动**，输入条工具行保持换行且模型下拉可截断，避免发送按钮被挤出屏外。
- **远程文件侧栏**：固定 460px 的右侧文件面板在手机上会溢出屏宽，改为**贴近视口全宽（可拖拽调整）**，不再越界。
- **登录/重登浮层**：小屏下缩小内边距、卡片限高并可**内部滚动**——软键盘弹出后字段仍可触及。

**AbortSignal 兼容**：DSH 客户端以 `AbortSignal.any([AbortSignal.timeout(ms), signal])` 合并 RPC 超时与调用方的中止信号；苹果设备上的 Chrome（实为 WKWebView）以及 Safari < 17.4 没有 `AbortSignal.any`，导致**每次发送消息都抛“AbortSignal.any is not a function”失败**。插件在客户端模块加载时安装规范兼容的 `AbortSignal.any`（含 `.timeout` 后备），全程不触碰 DSH 源码。
...
- **移动端 UX**（见上）：触屏回车=换行防误发、窄视口会话头部/输入条样式注入、远程文件侧栏手机全宽可调、登录/重登浮层小屏可滚动；
- **AbortSignal polyfill**：模块加载时补齐 `AbortSignal.any` / `.timeout`，修复苹果设备 Chrome（WKWebView）/ 旧 Safari 发送消息报“AbortSignal.any is not a function”；
